  ...
  .addEventListener('submit', function (event) {
    ...
    for (var i = 0; i < tutorCards.length; i++) {
      var price = parseFloat(
        tutorCards[i].querySelector('.tutor-price p').textContent
      );
      var subject = tutorCards[i]
        .querySelector('.tutor-info strong')
...
function applyFilter() {
  ...
  for (var i = 0; i < tutorCards.length; i++) {
    var priceText =
      tutorCards[i].getElementsByClassName('tutor-price')[0].innerText;
    ...
    var subject = tutorCards[i]
      .getElementsByClassName('tutor-info')[0]
    ...
    tutorCards[i].style.display =
      priceCondition && subjectCondition ? '' : 'none';
  }
}
